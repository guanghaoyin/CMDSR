...
if __name__ == '__main__':
    # from tools.utils import utils_model
    model = SRMD(in_nc=19, out_nc=3, nc=128, nb=12, upscale=4, act_mode='R', upsample_mode='pixelshuffle')
    model.cal_params()
    # print(utils_model.describe_model(model))

    x = torch.randn((2, 3, 100, 100))
    k_pca = torch.randn(2, 16, 1, 1)
    x = model(x, k_pca)
    print(x.shape)
