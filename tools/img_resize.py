"""
This code is copied from https://github.com/sefibk/KernelGAN/blob/master/imresize.py
"""
...
if __name__ == "__main__":
    ...
    fake_im = np.random.randint(255, size=[500, 500, 3], dtype=np.uint8)
    kernel = np.array(loadmat("yoon/kernels/track1/Flickr2K_000001/Flickr2K_000001_kernel_x2.mat")["Kernel"])
    # kernel = np.ones([17, 17]) / (17 * 17)
    fake_lr = imresize(fake_im, 0.5, kernel=kernel)
    cv2.imshow("fake", fake_im)
    cv2.imshow("lr", fake_lr)
    cv2.waitKey()
    print("fin.")